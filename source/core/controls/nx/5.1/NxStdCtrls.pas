...
procedure TNxGlyphSettings.Assign(Source: TPersistent);
begin
  if Source is TNxGlyphSettings then
  begin
    NormalGlyph.Assign(TNxGlyphSettings(Source).NormalGlyph);
    Transparent := TNxGlyphSettings(Source).Transparent;
    TransparentColor := TNxGlyphSettings(Source).TransparentColor;
  end else inherited;
end;
