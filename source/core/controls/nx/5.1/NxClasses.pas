...
const
  clVistaDialog = $00f0f0f0;
  clVistaDialogDivider = $00dfdfdf;
...
type
  ...
  TNxItemState = set of (isSelected, isFocused);
  ...
  TMemoDisplayOptions = (mdDefault, mdContent);
...
{$ENDIF}
  ...
  TDayNamesArray = array[1..7] of string;
  ...
  public
    ...
    property LongDayNames[Index: Integer]: string read GetLongDayNames;
  ...
  function NumberKey(Key: Word): Integer;
  ...
  function IsPointNil(Point: TPoint): Boolean;
  function ValueFromIndex(Strings: TNxStrings; Index: Integer): TString;
  function ValueFromPair(S: WideString): TString;
...
begin
  ...
  begin
    if not (S[i] in ['0'..'9', LocalSettings.DecimalSeparator, '-', '+'])
      or ((S[i] in ['-', '+']) and (i > 1)) then
    begin
      Check := [];
    end else
    begin
      if S[i] = LocalSettings.DecimalSeparator then
      begin
        Exclude(Check, isInt);
        Inc(Signs);
...
begin
  ...
  if UnicodeSupported then
    DrawTextExW(Canvas.Handle, PWideChar(Text), -1, CR, Flags or DT_CALCRECT, nil)
      ...
      {$ElSE}
      else Windows.DrawTextEx(Canvas.Handle, PWideChar(StringText), -1, CR, Flags or DT_CALCRECT, nil);
      {$ENDIF}
...
begin
  ...
  repeat
    if S[i] in ['0'..'9', LocalSettings.DecimalSeparator, '-', '+', #8] then
    begin
...
begin
  if Index >= 0 then
    Result := Copy(Strings[Index], Length(Strings.Names[Index]) + 2, MaxInt) else
    Result := '';
end;
...
begin
  P := Pos(S, '=');
  Result := Copy(S, P + 2, MaxInt);
end;

{ TNxFormatSettings }

function TLocalSettings.GetDateSeparator: Char;
begin
{$IFDEF FORMAT_SETTINGS}
...
function TLocalSettings.GetDecimalSeparator: Char;
begin
{$IFDEF FORMAT_SETTINGS}
...
function TLocalSettings.GetListSeparator: Char;
begin
{$IFDEF FORMAT_SETTINGS}
...
function TLocalSettings.GetLongDayNames(Index: Integer): string;
begin
{$IFDEF FORMAT_SETTINGS}
  Result := FormatSettings.LongDayNames[Index];
{$ELSE}
  Result := LongDayNames[Index];
{$ENDIF}
end;

function TLocalSettings.GetLongMonthNames(Index: Integer): string;
begin
{$IFDEF FORMAT_SETTINGS}
  Result := FormatSettings.LongMonthNames[Index];
{$ELSE}
  Result := LongMonthNames[Index];
{$ENDIF}
end;

function TLocalSettings.GetLongTimeFormat: string;
begin
{$IFDEF FORMAT_SETTINGS}
...
function TLocalSettings.GetShortDayNames(Index: Integer): string;
begin
{$IFDEF FORMAT_SETTINGS}
  Result := FormatSettings.ShortDayNames[Index];
{$ELSE}
  Result := ShortDayNames[Index];
{$ENDIF}
end;

function TLocalSettings.GetTimeSeparator: Char;
begin
{$IFDEF FORMAT_SETTINGS}
