...
type
  ...
  private
    ...
    function GetRealPosition: Integer;
  protected
    function IsVisible: Boolean;
    procedure SetVisible(const Value: Boolean); virtual;
    procedure UpdateScrollBar; virtual;
    property RealPosition: Integer read GetRealPosition;
  public
    ...
    procedure SetValues(AMax, APageSize, ALargeChange: Integer); virtual; 
  ...
  protected
    ...
    function CanScroll(Bar: TNxScrollBar; ScrollCode: Word): Boolean;
    procedure CMEnabledChanged(var Message: TMessage); message CM_ENABLEDCHANGED;
    procedure CreateParams(var Params: TCreateParams); override;
    procedure CreateWnd; override;
    procedure DoHorizontalScroll(Position: Integer); dynamic;
    function DoMouseWheelDown(Shift: TShiftState; MousePos: TPoint): Boolean; override;
    function DoMouseWheelUp(Shift: TShiftState; MousePos: TPoint): Boolean; override;
    procedure DoRollTimer(Sender: TObject);
    procedure DoVerticalScroll(Position: Integer); dynamic;
    procedure MouseDown(Button: TMouseButton; Shift: TShiftState; X, Y: Integer); override;
    procedure MouseMove(Shift: TShiftState; X, Y: Integer); override;
    procedure MouseUp(Button: TMouseButton; Shift: TShiftState; X, Y: Integer); override;
...
function TNxCustomScrollBar.GetRealPosition: Integer;
var
  SI: TScrollInfo;
  Code: Integer;
begin
  ...
  if Kind = skVertical then Code := SB_VERT else Code := SB_HORZ;
  GetScrollInfo(FOwner.Handle, Code, SI);
  Result := SI.nTrackPos;
end;
...
begin
	if Max > 0 then
		case Kind of
	    skHorizontal: if Position > 0 then SendMessage(FOwner.Handle, WM_HSCROLL, SB_LINELEFT, 0);
	    skVertical: if Position > 0 then SendMessage(FOwner.Handle, WM_VSCROLL, SB_LINEUP, 0);
	  end;
end;
...
function TNxCustomScrollBar.IsVisible: Boolean;
begin
  Result := (FMax > FPageSize) or not AutoHide;
end;

procedure TNxCustomScrollBar.SetValues(AMax, APageSize,
  ALargeChange: Integer);
begin
  FMax := AMax;
  FPageSize := APageSize;
  FLargeChange := ALargeChange;
...
begin
  ...
  IsUpdating := True;
  { Write Current position to determine if is scrolled }
  ...
  if CurrentPosition <> FPosition then
    if FOwner is TNxScrollControl then
      with FOwner as TNxScrollControl do
      begin
        if Kind = skHorizontal
          then DoHorizontalScroll(FPosition)
          else DoVerticalScroll(FPosition);
      end;
...
procedure TNxScrollControl.DoVerticalScroll(Position: Integer);
begin
...
begin
  ...
  DoHorizontalScroll(FHorzScrollBar.Position);
end;
...
begin
  ...
  DoVerticalScroll(VertScrollBar.Position);
end;

procedure TNxScrollControl.DoHorizontalScroll(Position: Integer);
begin
...
function TNxScrollControl.CanScroll(Bar: TNxScrollBar;
  ScrollCode: Word): Boolean;
begin
  { Def. can scroll }
  Result := True;

  case ScrollCode of
    SB_LINEDOWN, SB_PAGEDOWN, SB_BOTTOM: Result := not Bar.IsLast;
    SB_LINEUP, SB_PAGEUP, SB_TOP: Result := not Bar.IsFirst;
  end;
end;
