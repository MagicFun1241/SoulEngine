...
type
  ...
  procedure BitmapInRect(Canvas: TCanvas; Dest: TRect; Bitmap: TBitmap);
...
//  function MixColors(FG, BG: TColor; T: byte): TColor;
  ...
  procedure DrawVistaToolbar(Canvas: TCanvas; ARect: TRect; FromColor, ToColor: TColor);
...
procedure BitmapInRect(Canvas: TCanvas; Dest: TRect; Bitmap: TBitmap);
	...
	var
		i, j, Right, Bottom: Integer;
	  c: TColor;
	  P, P2, P3: PByte;
    r, g, b, r2, g2, b2, level: Byte;
    bmp, back: TBitmap;
	begin
    if Bitmap.Width < Dest.Right - Dest.Left then Right := Bitmap.Width else Right := Dest.Right - Dest.Left;
    if Bitmap.Height < Dest.Bottom - Dest.Top then Bottom := Bitmap.Height else Bottom := Dest.Bottom - Dest.Top;
    ...
    back.Width := Right;
    back.Height := Bottom;
    back.Canvas.CopyRect(Rect(0, 0, Right, Bottom), Canvas, Rect(Dest.Left, Dest.Top, Dest.Left + Right, Dest.Top + Bottom));
	  ...
	  Canvas.Draw(Dest.Left, Dest.Top, bmp);
    bmp.Free;
    back.Free;
  end;
...
begin
    ...
    pf24bit: Draw24BitBitmap;
  	else Canvas.Draw(Dest.Left, Dest.Top, Bitmap);
  end;
end;
...
//	    	c := TMaskProvider.MixColors(RGB(r, g, b), BackgroundColor, level);
	    	c := BlendColor(RGB(r, g, b), BackgroundColor, level);
        PutColorValues32(P2, GetRValue(c), GetGValue(c), GetBValue(c));
			end;
	  end;
	  Canvas.Draw(X, Y, bmp);
...
var
  ...
  c: TColor;
  P: PByte;
  am, am2, GlowHeight: Integer;
begin
  GlowHeight := (ARect.Bottom - ARect.Top) div 2;
