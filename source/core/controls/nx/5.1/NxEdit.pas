...
type
  ...
  public
    ...
    function CanDispatchKey(Key: Word): Boolean; virtual;
    ...
    property EditInfo: TNxEditInfo read FEditInfo;
  ...
  private
    ...
    procedure SetTextBefore(const Value: TCaption);
  ...
  public
    ...
    property TextBefore: TCaption read FTextBefore write SetTextBefore;
  ...
  public
    ...
    destructor Destroy; override;
    procedure BeginEditing; override;
    property Button: TNxSmallButton read FButton;
  published
  ...
  public
    constructor Create(AOwner: TComponent); override;
    destructor Destroy; override;
    property PictureDialog: TOpenPictureDialog read FPictureDialog write FPictureDialog;
  end;
  ...
  published
    property DrawingOptions: TDrawingOptions read FDrawingOptions write FDrawingOptions default doNormal;
  ...
  private
    ...
    function SelectItem(var AnItem: String): Boolean;
  ...
  protected
    ...
    procedure DoDrawItem(Sender: TObject; Index: Integer; Rect: TRect; State: TNxItemState); dynamic;
  ...
  published
    ...
    property AutoCompleteDelay: Cardinal read FAutoCompleteDelay write FAutoCompleteDelay default 750;
  ...
  protected
    ...
    procedure UpdateSpinButtonsRect;
    procedure UpdateEditMargins(var LeftMargin, RightMargin: Integer); override;
...
{$HINTS ON}
  ...
  public
  	constructor Create(AOwner: TComponent); override;
    function CanDispatchKey(Key: Word): Boolean; override;
  published
  ...
  TNxDayFontEvent = procedure(Sender: TObject; Day, Month, Year: Integer; Font: TFont) of object;
  ...
  protected
    ...
    procedure DoDayFont(Day, Month, Year: Integer; Font: TFont); dynamic;
    procedure MouseDown(Button: TMouseButton; Shift: TShiftState; X, Y: Integer); override;
    procedure MouseMove(Shift: TShiftState; X, Y: Integer); override;
    procedure MouseUp(Button: TMouseButton; Shift: TShiftState; X, Y: Integer); override;
    procedure Paint; override;
    procedure PaintDay(Day, Month, Year: Integer; DayRect: TRect);
  ...
  published
    ...
    property UseStandardMinute: Boolean read FUseStandardMinute write SetUseStandardMinute;
  end;
...
begin
  ...
  repeat
    if UnCheckedText[i] in ['0'..'9', LocalSettings.DecimalSeparator, '-', '+', #8] then
    begin

      { Decimal Separator }
      if UnCheckedText[i] = LocalSettings.DecimalSeparator then
      begin
        if (not AllowFloat)
  ...
  begin
    ...
    begin
      if BoundValue > Max then BoundValue := Max;
      if BoundValue < Min then BoundValue := Min;
    end;
  end else BoundValue := Min;
...
begin
  ...
  begin
    ...
    ApplyBitmap(Canvas, X, GlyphY, FOwner.Glyph);
...
begin
  ...
  begin
    ...
    ThemeRect(Handle, DC, R, NxThemesSupport.teEdit, 1, 0);
    ReleaseDC(Handle, DC);
  end;
end;
...
begin
  if Text <> '' then Result := StrToDateTime(Text) else Result := 0;
end;
...
begin
  if Text = '' then Result := 0 else Result := StrToInt(Text);
end;
...
function TNxCustomEdit.CanDispatchKey(Key: Word): Boolean;
begin
  Result := True;
end;
...
{$IFDEF CUSTOM_STYLES}
  UpDetail, DownDetail: TThemedScrollBar;
{$ENDIF}
begin
  inherited;
  ...
  begin
    ...
    begin
      if sbUp in FPressedButtons then UpDetail := tsArrowBtnUpPressed
        else if sbUp in FHoverButtons then UpDetail := tsArrowBtnUpHot
          else UpDetail := tsArrowBtnUpNormal;
      if sbDown in FPressedButtons then DownDetail := tsArrowBtnDownPressed
        else if sbDown in FHoverButtons then DownDetail := tsArrowBtnDownHot
          else DownDetail := tsArrowBtnDownNormal;
    end else
    begin
      UpDetail := tsArrowBtnUpDisabled;
      DownDetail := tsArrowBtnDownDisabled;
    end;
    StyleServices.DrawElement(Canvas.Handle, StyleServices.GetElementDetails(UpDetail), UpButtonRect);
    StyleServices.DrawElement(Canvas.Handle, StyleServices.GetElementDetails(DownDetail), DownButtonRect);
    {$ENDIF}
  end
    ...
    ThemeRect(Handle, Canvas.Handle, UpButtonRect, NxThemesSupport.teSpin, tcSpinUp, UpState);
    ThemeRect(Handle, Canvas.Handle, DownButtonRect, NxThemesSupport.teSpin, tcSpinDown, DownState);
  end else
  begin { theme off }
...
begin
  ...
  TGraphicsProvider.DrawWrapedTextRect(Canvas, TextRect, taLeftJustify, VerticalAlignment, False, Caption, bdLeftToRight);
end;
...
procedure TNxCheckBox.CMEnabledChanged(var Message: TMessage);
begin
  inherited;
  Invalidate;
end;
...
begin
  ...
  begin
    ...
    ThemeRect(Handle, Canvas.Handle, Result, NxThemesSupport.teButton, tcCheckBox, Index);
  end else
  begin
...
function TNxCheckBox.CanAutoSize(var NewWidth,
  NewHeight: Integer): Boolean;
begin
  inherited CanAutoSize(NewWidth, NewHeight);
  NewWidth := 11 + 8 + GetTextWidth(Canvas, Caption) + FIndent;
  NewHeight := 11 + 8;
  Result := True;
end;
...
//moved    ItemIndex := Self.ItemIndex;
    ...
    if Items.Count <= Self.DropDownCount
      then ClientHeight := Items.Count * Self.ItemHeight
...
procedure TNxComboBox.KeyPress(var Key: Char);
  ...
  begin
    Result := False;
    if Style in [dsDropDown] then
    begin
      SendMessage(Self.Handle, EM_GETSEL, Integer(@StartPos), Integer(@EndPos));
...
begin
  ...
  if Style in [dsDropDown] then FFilter := Text else
  begin
  ...
  case Ord(Key) of
    ...
    VK_TAB: if FAutoDropDown and DroppedDown then DroppedDown := False;
    VK_BACK:
      if FAutoComplete then
      begin
        if HasSelectedText(StartPos, EndPos) then DeleteSelectedText else
        begin
          if (Style in [dsDropDown]) and (Length(Text) > 0) then
          begin
            SaveText := Text;
            LastByte := StartPos;
            ...
            SendMessage(Handle, EM_SETSEL, 0, MakeLParam(LastByte - 1, LastByte - 1));
            FFilter := Text;
          end else
          begin
      ...
      if FAutoComplete then
      begin
        if HasSelectedText(StartPos, EndPos) then SaveText := Copy(FFilter, 1, StartPos) + Key
          else SaveText := FFilter + Key;

        if Key in LeadBytes then
        begin
          if PeekMessage(Msg, Handle, 0, 0, PM_NOREMOVE) and (Msg.Message = WM_CHAR) then
          begin
            SaveText := SaveText + Char(Msg.wParam);
        ...
        end else if SelectItem(SaveText) then Key := #0
      end; { FAutoComplete }
    end; { else }
end;
...
begin
  if (Items.Count > 0) and InRange(ItemIndex, 0, Pred(Items.Count)) then
    Result := ValueFromIndex(Items, ItemIndex) else Result := '';
end;
...
begin
  if IndexInRange(FItemIndex) then
    case FDisplayMode of
      ...
      dmValueList: Text := ValueFromIndex(Items, FItemIndex);
      dmNameList: Text := Items.Names[FItemIndex];
      dmIndentList: Text := ValueFromIndex(Items, FItemIndex);
    end;
//   else Text := '';
end;
...
procedure TNxComboBox.DoDrawItem(Sender: TObject; Index: Integer; Rect: TRect;
  State: TNxItemState);
begin
  if Assigned(FOnDrawItem) then FOnDrawItem(Self, Index, Rect, State);
end;

function TNxComboBox.SelectItem(var AnItem: String): Boolean;
var
  Idx: Integer;
  ValueChange: Boolean;
begin
  ...
  for Idx := 0 to  Self.FItems.Count - 1 do
  begin
    case Self.DisplayMode of
      dmValueList:
      begin
        if AnsiStartsText(AnItem, GetValueFromIndex(Idx)) then
          Break;
      end;
      else
      begin
        if AnsiStartsText(AnItem, Self.FItems[Idx]) then
          Break;
      end;
    end;
  end;
  If Idx >= Self.FItems.Count then Idx := CB_ERR;
  ...
  case Self.DisplayMode of
    dmDefault:
    begin
      AnItem := Copy(Self.Items[Idx], 1, Length(AnItem));   // make AnItem match the itemList entry
      if (Style in [dsDropDown]) then
      begin
        Text := AnItem + Copy(Items[Idx], Length(AnItem) + 1, MaxInt);
        SendMessage(Handle, EM_SETSEL, Length(AnItem), MaxInt);
      end
      else
      begin
        ItemIndex := Idx;
    ...
    dmValueList:
    begin
      AnItem := Copy(GetValueFromIndex(Idx), 1, Length(AnItem));   // make AnItem match the itemList entry
      if (Style in [dsDropDown]) then
      begin
        Text := AnItem + Copy(GetValueFromIndex(Idx), Length(AnItem) + 1, MaxInt);
        SendMessage(Handle, EM_SETSEL, Length(AnItem), MaxInt);
      end
      else
      begin
...
begin
  ...
  begin
    ...
    { Width & Height set inside }
    BeforePopup;

    TodayButton.Width := Width div 3;
    TodayButton.Height := DaySize.cy + 6;

    NoneButton.Width := Width div 3;
    NoneButton.Height := DaySize.cy + 6;

    NoneButton.Visible := ShowNoneButton;

    if NoneButton.Visible then
    begin
      TodayButton.Left := Width div 3 div 2 - 5;
      NoneButton.Left := Width div 3 * 3 div 2 + 5;
    end
    else TodayButton.Left := ClientWidth div 2 - TodayButton.Width div 2;

    TodayButton.Top := Height - TodayButton.Height - 8;
    NoneButton.Top := Height - NoneButton.Height - 8;
...
begin
  ...
  begin
    case Key of
      ...
      VK_PRIOR: if (ssShift in Shift) then Date := IncYear(Date, -1)
        else Date := IncMonth(Date, -1);
      VK_NEXT: if (ssShift in Shift) then Date := IncYear(Date, 1)
        else Date := IncMonth(Date, 1)
...
procedure TNxDatePicker.WMPaint(var Message: TWMPaint);
begin
  inherited;
  ...
  begin
    ...
    DrawText(Canvas, ClientRect, taLeftJustify, EmptyText);
  end;
end;
...
begin
  ...
  if not (Key in ['0'..'9', LocalSettings.DecimalSeparator, '-', '+', #1, #8, #13]) then Key := #0;

  { Decimal Separator }
  if Key = LocalSettings.DecimalSeparator then
...
begin
  ...
  AllowFloat := (eoAllowFloat in FOptions)
    and (Pos(LocalSettings.DecimalSeparator, ClientText) = 0);
...
begin
  ...
  begin
    ...
    begin
      if Precision > 0 then
        Result := FTextBefore + FloatToStrF(AValue, ffFixed, 18, Precision) + FTextAfter
          else Result := FTextBefore + FloatToStr(AValue) + FTextAfter;
    end else Result := FTextBefore + FloatToStrF(AValue, ffFixed, 18, 0) + FTextAfter;
  end;
end;
...
procedure TNxCustomNumberEdit.SetTextBefore(const Value: TCaption);
begin
  FTextBefore := Value;
  if not (eoAllowAll in FOptions)
    then SetValueToText(Self.Value);
...
begin
  ...
  { Left Button }
  FButton := TNxExecuteButton.Create(Self);
  FButton.Parent := Self;
  FButton.Visible := False;
  FButton.CellEditor := Self;
  FButton.OnButtonClick := DoButtonClick;
...
begin
  ...
  if not Focused and Visible then SetFocus;
...
begin
  ...
  if (csDesigning in ComponentState)
    and not ShowButton then
  begin
    SetWindowPos(FButton.Handle, HWND_TOP, -FButton.Width, 0, FButton.Width, 0, SWP_HIDEWINDOW);

    Exit;
  end;

  SetWindowPos(FButton.Handle, HWND_TOP, 0,
    0, FButton.Width, Self.ClientHeight, ButtonVisible[ShowButton] or SWP_NOREDRAW);
  FButton.Repaint;
end;

procedure TNxSpinEdit.UpdateSpinButtonsRect;
const
...
begin
  inherited;
  Repaint;
  if ShowButton then FButton.Repaint;
  if SpinButtons then FButtons.Repaint;
end;
...
begin
  ...
  if FButton.Visible then Inc(LeftMargin, FButton.Width);
end;

procedure TNxSpinEdit.SetShowButton(const Value: Boolean);
const
  ButtonVisible: array[Boolean] of Integer = (SW_HIDE, SW_SHOW);
begin
  FShowButton := Value;
  ShowWindow(FButton.Handle, ButtonVisible[FShowButton]);
  FButton.Visible := FShowButton;
  UpdateEditRect;
  UpdateButtonRect;
end;

procedure TNxSpinEdit.SetButtonWidth(const Value: Integer);
begin
  FButtonWidth := Value;
  FButton.Width := Value;
  UpdateEditRect;
...
procedure TNxSpinEdit.DoButtonClick(Sender: TObject);
begin
  if Assigned(FOnButtonClick) then FOnButtonClick(Self);
end;

procedure TNxSpinEdit.SetButtonCaption(const Value: WideString);
begin
...
begin
  ...
  repeat
    if UnCheckedText[i] in ['0'..'9', 'a'..'f', 'A'..'F', '#', '$', 'x', #8] then
    begin
      { Prefix }
      if (UnCheckedText[i] in ['#', '$']) then
  ...
  if Pos('0x', UnCheckedText) <> 1 then
  begin
    { Add prefix }
    if AddPrefix and (Pos(WebColorPrefix[WebColorFormat], UnCheckedText) = 0)
      then Insert(WebColorPrefix[WebColorFormat], UnCheckedText, 0);
  end;
...
procedure TNxExecuteButton.CreateWnd;
begin
  inherited;
  if IsThemed then FTheme := OpenThemeData(Handle, NxThemesSupport.teButton);
end;
...
begin
  ...
  begin
    ...
    if Hover then Detail := tbPushButtonHot;
    if Down then Detail := tbPushButtonPressed;
    if not TControl(Owner).Enabled then Detail := tbPushButtonDisabled;
    StyleServices.DrawElement(Canvas.Handle, StyleServices.GetElementDetails(Detail), ClientRect);
    {$ENDIF}
  end
	else if IsThemed then
 	begin
    ...
    r := ClientRect;
    InflateRect(r, 1, 1);
    DrawThemeBackground(FTheme, Canvas.Handle, 1, State, r, nil);
  end else
  begin { theme off }
    with Canvas do
    begin
...
procedure TNxExecuteButton.WMThemeChanged(var Message: TMessage);
begin
  if IsThemed then
  begin
    CloseThemeData(FTheme);
    FTheme := OpenThemeData(Handle, NxThemesSupport.teButton);
  end;
end;
...
procedure TDropDownButton.CreateWnd;
begin
  inherited;
  if IsThemed then FTheme := OpenThemeData(Handle, NxThemesSupport.teComboBox);
end;
...
begin
  ...
  begin
    {$IFDEF CUSTOM_STYLES}
    Detail := tcDropDownButtonNormal;
    if Hover then Detail := tcDropDownButtonHot;
    if Down then Detail := tcDropDownButtonPressed;
    if not TControl(Owner).Enabled then Detail := tcDropDownButtonDisabled;
    StyleServices.DrawElement(Canvas.Handle, StyleServices.GetElementDetails(Detail), ClientRect);
    {$ENDIF}
  end
	else if IsThemed then
    ...
    if Down then State := 3 else if FHover then State := 2 else State := 1;
    if not TControl(Owner).Enabled then State := 4;
    DrawThemeBackground(FTheme, Canvas.Handle, 1, State, ClientRect, nil);
  end else
  begin { theme off }
...
procedure TDropDownButton.WMThemeChanged(var Message: TMessage);
begin
  if IsThemed then
  begin
    CloseThemeData(FTheme);
    FTheme := OpenThemeData(Handle, NxThemesSupport.teComboBox);
  end;
end;
...
procedure TNxPopupEdit.DoPaint;
begin
...
begin
  ...
  begin
    ...
    begin
      Font.Assign(Self.Font);
      if Focused and not HideFocus then
      begin
        Brush.Color := GetFillColor;
        if not InplaceEditor then Font.Color := clHighlightText;

        { DrawingOptions }
        if DrawingOptions <> doCustomOnly
          then FillRect(R);
      ...
      { DrawingOptions }
      if (DrawingOptions <> doBackgroundOnly) and (DrawingOptions <> doCustomOnly)
        then TGraphicsProvider.DrawWrapedTextRect(Canvas, TextR, Alignment, taAlignTop, False, Text, BiDiMode);
    end;
  end;
  DoPaint; { Event }
end;
...
constructor TNxImagePathEdit.Create(AOwner: TComponent);
begin
  inherited;
  FPictureDialog := TOpenPictureDialog.Create(Self);
end;

destructor TNxImagePathEdit.Destroy;
begin
...
begin
  ...
  if FTimeFormat = '' then Result := LocalSettings.LongTimeFormat else Result := FTimeFormat;
  Result := CleanTimeFormat(Result);
end;

function TNxTimeEdit.GetTimeSeparator: string;
begin
  if FTimeSeparator = '' then Result := LocalSettings.TimeSeparator else
...
begin
  ...
  if not (Key in ['0'..'9', LocalSettings.DecimalSeparator, '-', '+', #8]) then Key := #0;
...
begin
  Text := Text + LocalSettings.DecimalSeparator;
  MoveCursor;
end;
...
begin
  ...
  Canvas.Font.Assign(Font);
  if not Enabled then Canvas.Font.Color := clBtnFace;
...
begin
  ...
  begin
    ...
    if not Enabled then Inc(Index, 3);
    ThemeRect(Handle, Canvas.Handle, Result, NxThemesSupport.teButton, tcRadioButton, Index);
  end else
  begin
...
function TNxRadioButton.CanAutoSize(var NewWidth,
  NewHeight: Integer): Boolean;
begin
  inherited CanAutoSize(NewWidth, NewHeight);
  NewWidth := 11 + 8 + GetTextWidth(Canvas, Caption) + FIndent;
...
function TNxMemoInplaceEdit.CanDispatchKey(Key: Word): Boolean;
  ...
  begin
    Result := 0;
    if HandleAllocated or (WindowText <> nil) then
    begin
      Result := SendMessage(Handle, EM_GETLINECOUNT, 0, 0);
      if SendMessage(Handle, EM_LINELENGTH, SendMessage(Handle,
        EM_LINEINDEX, Result - 1, 0), 0) = 0 then Dec(Result);
    end;
  end;

  function CanDispatchArrow: Boolean;
  begin
    Result := ((Key = VK_DOWN) and (GetCaretPos.Y >= Pred(GetLinesCount)))
      or ((Key = VK_UP) and (GetCaretPos.Y = 0));
  end;
begin
  { Default }
  Result := True;
  ...
  case Key of
    VK_RETURN: Result := not FWantReturns;
    VK_LEFT..VK_DOWN: Result := CanDispatchArrow;
  end;
end;
...
procedure TNxMonthCalendar.DoDayFont(Day, Month, Year: Integer; Font: TFont);
begin
  if Assigned(FOnDayFont) then FOnDayFont(Self, Day, Month, Year, Font);
end;
...
begin
  ...
  case GetActualStartDay of
    dwSunday: Result := LocalSettings.LongDayNames[DayInWeek][1];
    dwMonday: if DayInWeek = 7 then Result := LocalSettings.LongDayNames[1][1]
      else Result := LocalSettings.LongDayNames[DayInWeek + 1][1];
  end;
end;
...
begin
  if Assigned(FMonthNames) and InRange(Month, 1, 12)
    and (FMonthNames.Count >= 12)
      then Result := FMonthNames[Month - 1] else Result := LocalSettings.LongMonthNames[Month];
end;
...
procedure TNxMonthCalendar.PaintDay(Day, Month, Year: Integer; DayRect: TRect);
var
  StringText: string;
begin
  ...
  begin
    ...
    DoDayFont(Day, Month, Year, Canvas.Font);
...
procedure TNxTimePicker.SetUseStandardMinute(const Value: Boolean);
begin
  FUseStandardMinute := Value;
...
begin   
  ...
  if Text <> '' then
  begin
    SetAsDateTime(StrToDateDef(Text, Time));
  end;
end;
