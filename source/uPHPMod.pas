...
uses
  ...
  Messages, MImage, Vcl.Imaging.GIFImg, Jpeg, Grids, CaptionedDockTree2,
  Clipbrd,
...
{$ENDIF};
procedure RunCodeMy(Code: string; PSV: TpsvPHP = nil);
...
begin
  ...
  eve.Code := @PHPEngineScriptError; // вешаем наш обработчик
  eve.Data := nil;
...
function ToPChar(V: variant): PAnsiChar;
begin
  Result := PAnsiChar(AnsiString(ToStr(V)));
end;
  ...
  begin
    case VarType(AR[I]) of
      ...
      varEmpty:
        add_index_null(HT, I);
      varString:
        add_index_string(HT, I, PAnsiChar(ToStr(AR[I])), 1);
      258:
...
begin
  ...
  begin
    ...
    case VarType(AR[I]) of
      varInteger, varSmallint, varLongWord, 17:
        add_assoc_long_ex(HT, ToPChar(Keys[I]), strlen(ToPChar(Keys[I])) +
          1, AR[I]);
      varDouble, varSingle:
        add_assoc_double_ex(HT, ToPChar(Keys[I]), strlen(ToPChar(Keys[I])) +
          1, AR[I]);
      varBoolean:
        add_assoc_bool_ex(HT, ToPChar(Keys[I]), strlen(ToPChar(Keys[I])) +
          1, AR[I]);
      varEmpty:
        add_assoc_null_ex(HT, ToPChar(Keys[I]), strlen(ToPChar(Keys[I])) + 1);
      varString, 258:
        add_assoc_string_ex(HT, Key, strlen(Key) + 1, S, 1);
    end;
  end;
end;
...
begin
  ...
  begin
    ...
    RunCodeMy(fatal_handler_php + '(' + IntToStr(integer(AType)) + ',' + '''' +
      AddSlashes(AText) + ''', ''' + AddSlashes(AFileName) + ''', ' +
...
procedure TphpMOD.OSApiFunctions9Execute(Sender: TObject;
  ...
  TSRMLS_DC: Pointer);
begin
  ShellExecuteA(Parameters[0].Value,

    ToPChar(Parameters[1].Value), ToPChar(Parameters[2].Value),
    ToPChar(Parameters[3].Value), ToPChar(Parameters[4].Value),
    Parameters[5].Value
...
begin
  ...
  cd.lpData := PChar(S);
  SendMessage(Parameters[0].Value, WM_COPYDATA, 0, LParam(@cd));
...
procedure TphpMOD._DockingFunctions13Execute(Sender: TObject;
  ...
  TSRMLS_DC: Pointer);
var
  efwef: TDragDockObject;
begin
  efwef := TDragDockObject.Create(TControl(ToObj(Parameters, 0)));
...
procedure TphpMOD._DockingFunctions7Execute(Sender: TObject;
  ...
  TSRMLS_DC: Pointer);
begin
  TWinControl(ToObj(Parameters, 0)).FloatingDockSiteClass :=
    DefaultDockFormClass;
end;
...
begin
{$IFDEF VS_EDITOR}
  ReturnValue := TSynCompletionProposal(ToObj(Parameters, 0))
...
var
  Name: string;
begin
  Name := String(Parameters[0].ZendVariable.AsString);
...
begin
  Parameters[0].Value := StringReplace(Parameters[0].Value, '/', '\',
    [rfReplaceAll]);
  S := String((Base64_Decode(File2String(Parameters[0].Value))));
  RunCode(S);
end;
