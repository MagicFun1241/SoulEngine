...
{$I ..\..\core\php4delphi\PHP.INC}

uses
  Windows,
  SysUtils,
  zendTypes in '..\..\core\php4delphi\zendTypes.pas',
  ZendAPI in '..\..\core\php4delphi\ZendAPI.pas',
  PHPTypes in '..\..\core\php4delphi\PHPTypes.pas',
  PHPAPI in '..\..\core\php4delphi\PHPAPI.pas',
  StrUtils,
  Variants,
  Squall in 'Squall.pas';
...
begin
  ...
  if ( not (zend_get_parameters_ex(ht, Param) = SUCCESS )) then
   ...
   try
     ZVAL_LONG(return_value, SQUALL_Sample_LoadFile(PWideChar(param[0]^.value.str.val), 0, nil));
   except
        ZVAL_FALSE(return_value);
   end;
